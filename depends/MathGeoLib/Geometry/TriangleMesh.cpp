...
TriangleMesh::~TriangleMesh() { AlignedFree(data); }
...
void TriangleMesh::Set(const Polyhedron &polyhedron) {
    ...
    if (!tris.empty()) {
        int alignment =
            (simdCapability == SIMD_AVX) ? 8 : ((simdCapability == SIMD_SSE41 || simdCapability == SIMD_SSE2) ? 4 : 1);
...
float TriangleMesh::IntersectRay_TriangleIndex_UV(const Ray &ray, int &outTriangleIndex, float &outU,
                                                  float &outV) const {
#ifndef MATH_AUTOMATIC_SSE // TODO: Restore support for this when MATH_AUTOMATIC_SSE is defined!
#ifdef MATH_AVX
    if (simdCapability == SIMD_AVX)
...
void TriangleMesh::SetSoA4(const float *vertexData, int numTris, int vtxSizeBytes) {
    ...
    assert(numTris % 4 ==
           0); // We must have an evenly divisible amount of triangles, so that the SoA swizzling succeeds.
...
void TriangleMesh::SetSoA8(const float *vertexData, int numTris, int vtxSizeBytes) {
    ...
    // From (xyz xyz xyz) (xyz xyz xyz) (xyz xyz xyz) (xyz xyz xyz) (xyz xyz xyz) (xyz xyz xyz) (xyz xyz xyz) (xyz xyz
    // xyz)
...
float TriangleMesh::IntersectRay_TriangleIndex_UV_CPP(const Ray &ray, int &outTriangleIndex, float &outU,
                                                      float &outV) const {
