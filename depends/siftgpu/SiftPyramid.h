...
#define FIRST_OCTAVE(V)            (V? _octave_num - 1 : 0)
#define NOT_LAST_OCTAVE(i, V)      (V? (i >= 0) : (i < _octave_num))
#define GOTO_NEXT_OCTAVE(i, V)     (V? (--i) : (++i))
#define FIRST_LEVEL(V)             (V? param._dog_level_num - 1 : 0)   
#define GOTO_NEXT_LEVEL(j, V)      (V? (--j) : (++j))
#define NOT_LAST_LEVEL(j, V)       (V? (j >= 0) : (j < param._dog_level_num))
#define FOR_EACH_OCTAVE(i, V)      for(int i = FIRST_OCTAVE(V); NOT_LAST_OCTAVE(i, V); GOTO_NEXT_OCTAVE(i, V))
#define FOR_EACH_LEVEL(j, V)       for(int j = FIRST_LEVEL(V);  NOT_LAST_LEVEL(j, V);  GOTO_NEXT_LEVEL(j, V))
#else
