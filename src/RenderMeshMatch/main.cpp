...
int main(int argc, char **argv) {
    ...
    options.add_options("RenderMeshMatch")
        ...
        ("b,block", "Path to a merged block, which helps to determine the ID of tiepoints", cxxopts::value(path_merged))
        ("r,results", "Load match results instead of create one, useful for merge block", cxxopts::value(path_results))
        ("h,help", "Print this help message");
    // clang-format on

    auto results = options.parse(argc, argv);
    if (results.arguments().empty() || results.count("help")) {
        std::cout << options.help({"RenderMeshMatch"}) << std::endl;
        return 1;
    }
    ...
    Eigen::Vector3d origin_coord = Eigen::Vector3d::Zero();

    if (!origin_coord_at.empty()) {
    ...
    if (path_results.empty()) {
        ...
        for (const auto &pgroups : block_ground_rectified.groups) {
            ...
            auto photos = tq::tqdm(pgroup.photos);
            photos.set_prefix("Photogroup");
            for (uint32_t iid : photos) {
                ...
                // set up matices and do the rendering
                {
                    ...
                    viewer.getCamera()->setProjectionMatrixAsPerspective(vfov, (double)width / (double)height, znear,
                                                                         zfar);
                ...
                // there are at most 30 levels, and each frame will request all the nodes in the next level
                for (int i = 0; i < 30; ++i) {
                    viewer.frame();
        ...
        match_results = load_matches(path_results);
    }

    {
        std::string dir = h2o::get_directory(path_ground_at);
        std::string name = h2o::get_filename_noext(path_ground_at);
        name = h2o::join_paths(dir, name + "_match.bin");
        save_matches(match_results, name);
    }

    if (!path_merged.empty()) {
        std::string dir = h2o::get_directory(path_merged);
        std::string name = h2o::get_filename_noext(path_merged);
        name = h2o::join_paths(dir, name + "_tiepoints.xml");
        // matcher.merge_block(match_results, path_merged, name);
        matcher.save_block(match_results, path_merged, name);
    }
